...
describe MathML::LaTeX::Parser do
	...
	def check_chr(tag, src)
		src.scan(/./) do |c|
			...
			expect(smml(c)).to match(/\A<#{tag_re}(\s+[^>]+)?>#{Regexp.escape(c)}<\/#{tag_re}>\z/)
		end
	end
	...
	def check_hash(tag, hash)
		hash.each do |k, v|
			tag_re = Regexp.escape(tag)
			expect(smml(k)).to match(/\A<#{tag_re}(\s+[^>]+)?>#{Regexp.escape(v)}<\/#{tag_re}>\z/)
		end
	end
	...
	describe "#parse" do
		...
		it "should process array" do
			...
			expect(math_ml('\left\{\begin{array}ca_b\end{array}\right\}')[0]).to match(EimXML::DSL.element(:mfenced, :open=>"{", :close=>"}"){
				element :mrow do
					element :mtable do
						element :mtr do
							element :mtd do
		...
		it "should parse \\left and \\right" do
			expect(math_ml('\left(\frac12\right)')[0]).to match(EimXML::DSL.element(:mfenced, :open=>"(", :close=>")"){
				element :mrow do
					element :mfrac do
						element(:mn).add("1")
						element(:mn).add("2")
			...
			expect(math_ml('\left \lfloor a\right \rfloor')[0]).to match(EimXML::DSL.element(:mfenced, :open=>EimXML::PCString.new("&lfloor;", true), :close=>EimXML::PCString.new("&rfloor;", true)){
				element :mrow do
					element(:mi).add("a")
				end
			})

			expect(math_ml('\left \{ a \right \}')[0]).to match(EimXML::DSL.element(:mfenced, :open=>"{", :close=>"}"){
				element :mrow do
					element(:mi).add("a")
				end
			})

			expect(math_ml('\left\{\begin{array}c\begin{array}ca\end{array}\end{array}\right\}')[0]).to match(EimXML::DSL.element(:mfenced, :open=>"{", :close=>"}"){
				element :mrow do
					element :mtable do
						element :mtr do
							element :mtd do
			...
			expect(math_ml('\left(\sum_a\right)')[0]).to match(EimXML::DSL.element(:mfenced, :open=>"(", :close=>")"){
				element :mrow do
					element :msub do
			...
			expect(math_ml('\left(\sum_a\right)', true)[0]).to match(EimXML::DSL.element(:mfenced, :open=>"(", :close=>")"){
				element :mrow do
					element :munder do
						element(:mo).add(EimXML::PCString.new("&sum;", true))
						element(:mi).add("a")
			...
			expect(math_ml('\left\|a\right\|')[0]).to match(EimXML::DSL.element(:mfenced, :open=>EimXML::PCString.new("&DoubleVerticalBar;", true), :close=>EimXML::PCString.new("&DoubleVerticalBar;", true)){
				element :mrow do
					element(:mi).add("a")
				end
			})
...
EOS
			...
			expect(math_ml('\begin{braced}{|}{)}\frac12\end{braced}', false, p)[0]).to match(EimXML::DSL.element(:mfenced, :open=>"|", :close=>")"){
				element(:mrow) do
					element(:mfrac) do
						element(:mn).add("1")
						element(:mn).add("2")
