...
describe MathML::LaTeX::Scanner do
	...
	it "#scan_block" do
		expect(new_scanner(" a").scan_block).to eq(nil)
		expect(new_scanner(" a").check_block).to eq(nil)
		...
		expect(new_scanner(' \command test').scan_block).to eq(nil)
		expect(new_scanner(' \command test').check_block).to eq(nil)

		expect(new_scanner("").scan_block).to eq(nil)
		expect(new_scanner("").check_block).to eq(nil)
	...
	it "#peek_command" do
		...
		expect(new_scanner("").peek_command).to eq(nil)
		expect(new_scanner(" ").peek_command).to eq(nil)
		expect(new_scanner(" a").peek_command).to eq(nil)
	end
