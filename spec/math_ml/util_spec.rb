...
describe MathML::Util::MathData do
	it "#<< and #update" do
		...
		expect(m.math_list).to eq(["ml1", "ml2"])
		expect(m.msrc_list).to eq(["sl1",  "sl2"])
		expect(m.dmath_list).to eq(["dml1", "dml2"])
		expect(m.dsrc_list).to eq(["dsl1", "dsl2"])
		expect(m.escape_list).to eq(["el1", "el2"])
		expect(m.esrc_list).to eq(["es1", "es2"])
		expect(m.user_list).to eq(["ul1", "ul2"])
		expect(m.usrc_list).to eq(["usl1", "usl2"])
	end
end
...
describe MathML::Util::SimpleLaTeX do
	...
	it "#decode_partial" do
		...
		s = MathML::Util::SimpleLaTeX.new(:delimiter=>%[$])
		encoded, data = s.encode("$a$")
		expect(s.decode_partial(:math, encoded, data)).to match(/^<math.*<\/math>/m)
	end
