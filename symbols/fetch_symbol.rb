...
def trace_list(com, h, d)
  ...
  if s.is_a?(String)
    ...
    until sc.eos?
      ...
      d << com if !d.include?(com) && d.include?(b)
    end
  end
...
def fetch_symbol(delims = [])
  ...
  f.scan(/\\def(\\[^{]+?)(\\.+)/) do |_m|
    raise StandardError, 'Uncaught commands'
  end
...
def output_list(h, preput = nil)
  ...
  sorted_symbol_list(h).each do |k|
    ...
    warn k unless p[k][0] == h[k]
  end

  warn '### Missing'
  p.each_key do |k|
    warn k unless h.include?(k)
  end
end
