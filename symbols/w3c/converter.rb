...
class CharacterCommandGenerator
  class Downloader
    ...
    def start
      ...
      URIS.each do |uri|
        filename = "#{HERE}/html/#{uri[%r{/([^/]*)\z}, 1]}"
        open(filename, 'w') { |f| f.print(get(uri)) } unless File.exist?(filename)
      end
  ...
  class Converter
    ...
    def parser(filename)
      ...
      Nokogiri.HTML(IO.read(filename)).search('pre')[0].text.each_line do |line|
        ...
        raise "OOPS: #{hash[name]} : #{string}" if hash.key?(name) && (hash.key?(name) && hash[name] != string)
